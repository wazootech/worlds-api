...
If you've used frameworks like Next.js, you might be familiar with file-based
routing — you add a file in a specific directory and it automatically becomes a
route. This tutorial demonstrates how to create a simple HTTP server that uses
file-based routing.
...
extracted from the request URL, and a `method` variable which contains the
request method.
...
This will start the server on `localhost:8000`. You can now make a `GET` request
to `localhost:8000/users` and you should see the response "Hello from user.ts".

This command requires the `--allow-net` and `--allow-read` permissions flags to
allow access to the network to start the server and to read the `users.ts` file

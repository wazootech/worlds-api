...
- **Automatic boot**: On startup, the console checks `data/workos.json` and
  spawns a `deno serve` process for every local organization.
...
session by setting environment variables in `packages/console/.env`:
...
The system uses these values to initialize the `data/workos.json` file if it
does not exist.
